...
def test_adicionar_item_ao_carrinho():
    resp_prod = client.post(
        "/products/",
        json={"name": "Sabonete", "price": 3.5, "quantity": 10, "setor": "Hortifruti"},
    )
    ...
    resp_item = client.post(
        f"/cart/{carrinho_id}/items", json={"produto_id": produto_id, "quantidade": 2}
    )
...
def test_adicionar_produto_inexistente():
    ...
    carrinho_id = resp_cart.json()
    resp = client.post(
        f"/cart/{carrinho_id}/items", json={"produto_id": 9999, "quantidade": 1}
    )
...
def test_adicionar_quantidade_invalida():
    resp_prod = client.post(
        "/products/",
        json={"name": "ItemX", "price": 5.0, "quantity": 10, "setor": "Limpeza"},
    )
    produto_id = resp_prod.json()["id"]
...
@pytest.mark.parametrize(
    "nome,preco,quantidade", [("Arroz", 10.0, 1), ("Feijão", 7.5, 3), ("Leite", 4.2, 5)]
)
def test_produtos_em_varios_carrinhos(nome, preco, quantidade):
    resp_prod = client.post(
        "/products/",
        json={"name": nome, "price": preco, "quantity": 50, "setor": "Padaria"},
    )
    assert resp_prod.status_code == 201
    produto_id = resp_prod.json()["id"]
